...
{{- fail "opsolving-app exposes one APISIX HTTP listener; additionalContainerPorts must be empty" -}}
{{- end -}}
{{- if .Values.apisix.service.stream.enabled -}}
{{- fail "opsolving-app does not permit APISIX TCP or UDP stream listeners" -}}
{{- end -}}
...
{{- fail "opsolving-app requires apisix.apisix.deployment.mode=standalone" -}}
{{- end -}}
{{- if ne .Values.apisix.apisix.deployment.role "data_plane" -}}
{{- fail "opsolving-app requires apisix.apisix.deployment.role=data_plane" -}}
{{- end -}}
{{- if ne .Values.apisix.apisix.deployment.role_data_plane.config_provider "yaml" -}}
{{- fail "opsolving-app requires the APISIX standalone YAML configuration provider" -}}
{{- end -}}
{{- if .Values.apisix.etcd.enabled -}}
{{- fail "opsolving-app standalone APISIX does not permit etcd" -}}
{{- end -}}
...
{{- fail "opsolving-app standalone APISIX does not permit external etcd configuration" -}}
{{- end -}}
{{- if (index .Values.apisix "ingress-controller").enabled -}}
{{- fail "opsolving-app does not permit the APISIX Ingress Controller or its CRDs" -}}
{{- end -}}
{{- if .Values.apisix.apisix.admin.enabled -}}
{{- fail "opsolving-app standalone APISIX does not permit the Admin API" -}}
{{- end -}}
{{- if .Values.apisix.apisix.admin.enable_admin_ui -}}
{{- fail "opsolving-app does not permit the APISIX Admin UI or dashboard" -}}
{{- end -}}
{{- if .Values.apisix.control.enabled -}}
{{- fail "opsolving-app standalone APISIX does not permit the Control API" -}}
{{- end -}}
{{- if .Values.apisix.apisix.prometheus.enabled -}}
{{- fail "opsolving-app exposes only the APISIX gateway Service; the APISIX metrics Service must remain disabled" -}}
{{- end -}}
{{- if .Values.apisix.ingress.enabled -}}
{{- fail "the APISIX subchart Ingress must remain disabled; use the opsolving-app exposure values" -}}
{{- end -}}
{{- if .Values.apisix.metrics.serviceMonitor.enabled -}}
{{- fail "opsolving-app does not permit the APISIX subchart ServiceMonitor" -}}
{{- end -}}
