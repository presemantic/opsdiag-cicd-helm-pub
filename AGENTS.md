...
Application exposure uses one direct `ingress` configuration, deterministic `openshiftRoute.api` and `openshiftRoute.front` configurations, and one `istio.gateway` plus one `istio.virtualService` configuration. Exposure values must not accept arbitrary resource `name`, per-resource `fullnameOverride`, or `items` wrappers. Every exposure resource name and every component backend reference must be generated from the Helm release and the chart-level `nameOverride` or `fullnameOverride`.
