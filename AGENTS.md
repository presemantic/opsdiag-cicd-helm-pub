...
The current unified App chart source version is `0.1.18` with `appVersion` `2026-07-21`. It includes the mandatory thin MCP Proxy and `/mcp` exposure, pinned to published image `2026-07-21.00-24-24.cb476b3` with the same current licenseclient contract as the other App services. Chart version changes do not authorize invented component image tags, and production deployment values must continue to avoid image overrides.
