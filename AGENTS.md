...
The current unified App chart source version is `0.1.22` with `appVersion` `2026-07-21`. It includes the mandatory authenticated MCP Proxy, OAuth/OIDC discovery and `/mcp/oauth/*` authorization routing, and `/mcp` exposure while reserving `/oauth` for App UI login. App API is pinned to `2026-07-21.21-33-40.c0a0890` and MCP Proxy to `2026-07-21.03-45-50.384afa0`; the App API consent CSP permits only the exact validated loopback callback origin for each native-client authorization request. Chart version changes do not authorize invented component image tags, and production deployment values must continue to avoid image overrides.
