...
The current unified App chart source version is `0.1.10` with `appVersion` `2026-07-20`. Chart version changes do not authorize invented component image tags: App Agent and App API defaults must move only to exact images that were actually released and attested, and production deployment values must continue to avoid image overrides.
